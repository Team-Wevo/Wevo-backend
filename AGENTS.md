...
- 실시간 협업(변경 반영·편집 잠금 등) 기능의 패키지 위치는 아직 미정입니다.
  구현 착수 전에 팀 합의로 위치(`realtime` 도메인 또는 `global` 하위)를 정하고 이 구조도를 함께 갱신합니다.
...
- **명시적 API 계약 예외**: 의견 수집 마감의 `SUBMITTED` 의견 0건(`O004`)은 노션 API 명세에 따라
  `422 Unprocessable Entity`로 응답한다. 마감 조건을 충족하지 못한 요청을 FE가 별도로 안내해야 하는
  Opinion 전용 계약이며, 구현·`ErrorCode`·API 명세를 같은 변경에서 동기화한다.
